    ...
    const point = Object.create({
        normalize() {
            ...
            return createPoint(this.x / length, this.y / length, this.z / length)
                .withTex(this.u, this.v);
        },
        withTex(u, v) {
            if (u != null && v != null) {
                const p = createPoint.clone(this);
                p.u = u;
                p.v = v;
                return p;
            }
...
createPoint.clone = (p) => createPoint(p.x, p.y, p.z);
...
export const model = {
    points: [
        // Front face
        createPoint(-1.0, -1.0,  1.0).normalize().withTex(0.0, 0.0),
        createPoint( 1.0, -1.0,  1.0).normalize().withTex(1.0, 0.0),
        createPoint( 1.0,  1.0,  1.0).normalize().withTex(1.0, 1.0),
        createPoint(-1.0,  1.0,  1.0).normalize().withTex(0.0, 1.0),
...
function buildSphere(quality, model) {
    ...
    return {
        ...
        generateTexCoords() {
            model.textureCoords = model.points.reduce((current, v) => current.concat(v.u, v.v), []);
            return this;
        },
        setCount() {
        ...
        getMiddlePoint(p1, p2) {
            ...
            const middle = createPoint.middleOf(point1, point2)
                .normalize()
                .withTex((point1.u + point2.u) / 2.0, (point1.v + point2.v) / 2.0);
            return middle;
        },
    };
}
