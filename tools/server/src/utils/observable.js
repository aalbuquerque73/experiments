...
export class Observable {
    constructor(value, factory = () => true) {
        ...
        this.defineProperty(res, 'map', { value: (...args) => this.map(...args), enumerable: true });
        this.defineProperty(res, 'each', { value: (...args) => this.each(...args), enumerable: true });
        ...
        this.defineProperty(res, 'call', { value: (...args) => this.callable(args), enumerable: true });
    ...
    map(map, forValue = false) {
        return new Observable(this._value, (observable) => {
            ...
            if (forValue) {
                observable._valueMappers.push(map);
            } else {
                observable._mappers.push(map);
            }
        });
    }
    ...
    each(listener) {
        return new Observable(this._value, (observable) => {
            ...
            observable._valueMappers = observable._valueMappers.concat(this._valueMappers);
            observable._subscribers = observable._subscribers.concat(this._subscribers);
            observable._subscribers.push(listener);
        });
    }

    filter(filter, after = false) {
        return new Observable(this._value, (observable) => {
            observable._filters = observable._filters.concat(this._filters);
            observable._afterFilters = observable._afterFilters.concat(this._afterFilters);
            observable._mappers = observable._mappers.concat(this._mappers);
            ...
            observable._subscribers = observable._subscribers.concat(this._subscribers);
            if (after) {
                observable._afterFilters.push(filter);
            } else {
                observable._filters.push(filter);
            }
        });
    }
    ...
    notify(value, oldValue) {
        this._subscribers.forEach(observer => observer(value, oldValue));
    }
