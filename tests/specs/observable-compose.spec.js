...
tap.test('observable', async (t) => {
    await t.test('compose', async (t) => {
        ...
        await t.test('initial value', (t) => {
            const compose = Observable.compose(value1, value2);
            t.plan(1);
        ...
        await t.test('subscribe', (t) => {
            ...
            Promise.resolve()
                ...
                .then(() => {
                    ...
                    t.ok(observer.calledOnce, 'compose observer should have been called once');
                    t.ok(observer.calledWith([2, 2], [1, 2]), 'compose observer should have been called with value ([2, 2], [1, 2])');
                })
                .then(() => value2(3))
                .then(() => delay(1))
                .then(() => {
                    t.deepEqual(compose.value, [2, 3], 'compose observable should have value [2, 3]');
                    t.ok(observer.calledTwice, 'compose observer should have been called twice');
                    t.ok(observer.calledWith([2, 3], [2, 2]), 'compose observer should have been called with value ([2, 3], [2, 2])');
                })
                .catch(console.error);
        });
        ...
        await t.test('unsubscribe', (t) => {
            ...
            Promise.resolve()
                ...
                .then(() => {
                    ...
                    t.ok(observer.calledOnce, 'compose observer should have been called once');
                    t.ok(observer.calledWith([1, 3], [2, 3]), 'compose observer should have been called with value ([1, 3], [2, 3])');
                })
                .then(() => dispose())
                .then(() => value2(2))
                .then(() => delay(1))
